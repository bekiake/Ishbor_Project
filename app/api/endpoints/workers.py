...
async def read_workers(
    ...
    for worker in workers:
        if worker.image:  # Agar rasm bo‘lsa
            worker.image = f"https://admin.ishbozor.uz{settings.MEDIA_URL}{worker.image}"
...
async def read_worker_me(
    ...
    worker_data = {
        ...
        "image": f"https://admin.ishbozor.uz{settings.MEDIA_URL}{worker.image}",
...
async def get_worker_detail(worker_id: int, db: AsyncSession = Depends(get_async_db)):
    ...
    workers.image = f"https://admin.ishbozor.uz{settings.MEDIA_URL}{workers.image}"
